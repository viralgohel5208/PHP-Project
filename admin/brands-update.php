...
if(isset($_POST['submit']))
{
	...
	if($error == "")
	{
		$query_2 = "UPDATE `brands` SET `brand_name` = '$brand_name', `file_name` = '$file_name', `updated_at` = '$current_datetime', `status` = '$status' WHERE id = $item_id";
		...
		else
		{
			...
			{
				if($old_file_name != "" && file_exists("../uploads/store-".$app_id."/brands/" . $old_file_name))
				{
					unlink("../uploads/store-".$app_id."/brands/" . $old_file_name);
				}
				//else { echo $old_file_name; exit; }
				move_uploaded_file($image_tmp_name, "../uploads/store-".$app_id."/brands/" . $file_name);
			}

			if($rm_file_name == 1 && $old_file_name != "" && file_exists("../uploads/store-".$app_id."/brands/".$old_file_name))
			{
				unlink( "../uploads/store-".$app_id."/brands/".$old_file_name );
			}

			$success = "Brand details has been updated successfully";
			unset($_POST);
			unset($_FILES);
		}
	}
...
if(isset($_GET[ 'id' ] ) )
{
	...
	$query_3 = "SELECT * FROM `brands` WHERE id = '" . $item_id . "'" ;
	...
	if($result_3)
	{
		if ( mysqli_num_rows( $result_3 ) > 0 )
		{
			...
			if($app_id != $app_id_db)
			{
				$_SESSION['msg_error'] = "Brand does't found";
...
<body class="form-inputs-page">
							...
							<div class="panel-body">
								<form class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
									<div class="form-group">
										...
										<div class="col-lg-7">
											<input type="text" id="" class="form-control" name="brand_name" value="<?php if(isset($_POST['brand_name'])) { echo $_POST['brand_name'];} else { echo $row_3['brand_name']; } ?>">
										</div>
									</div>
									<?php if($row_3['file_name'] != "" && file_exists("../uploads/store-".$app_id."/brands/".$row_3['file_name'])){ ?>
									<div class="form-group">
										<div class="col-lg-offset-3 col-lg-7">
											<img src="../uploads/store-<?= $app_id ?>/brands/<?php echo $row_3['file_name']; ?>" width="100px" class=""/>
	...
	<form id="delete_form" action="brands.php" method="post" style="display: hidden">
