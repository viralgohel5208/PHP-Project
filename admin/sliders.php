...
if(isset($_POST['delete_id']))
{
	...
	$query_3 = "SELECT id, app_id, file_name FROM `sliders` WHERE id = '" . $id . "'" ;
	...
	if($result_3)
	{
		if ( mysqli_num_rows( $result_3 ) > 0 )
		{
			...
			if($app_id != $app_id_db)
			{
				$_SESSION['msg_error'] = "Slider does't found";
			}
			else
			{
				if($file_name != "" && file_exists("../uploads/store-".$app_id."/sliders/" . $file_name))
				{
					@unlink("../uploads/store-".$app_id."/sliders/".$file_name);
				}

				$result = "DELETE FROM sliders WHERE id = '" . $id . "'";
			...
			<header id="topbar">
				<div class="topbar-left">
					<ol class="breadcrumb">
						<li class="crumb-active">
							<a href="sliders.php"><?php echo $page_title; ?></a>
						</li>
						<li class="crumb-icon">
				...
				<div class="topbar-right">
					<div class="ml15 ib va-m" id="">
						<a href="sliders-add.php" class="pl5 btn btn-default btn-sm">
							<i class="fa fa-plus"></i> Add New
						</a>
						<a href="sliders-sort.php" class="pl5 btn btn-default btn-sm" style="margin-left: 20px;">
			...
			<section id="content">
						...
						<div class="panel <?php echo $panel_style; ?>">
							<div class="panel-heading">
								<span class="panel-title">
									<span class="fa fa-picture-o"></span><?php echo $page_title; ?>
								</span>
								<div class="pull-right hidden-xs">
									<a href="sliders-add.php" class="btn btn-xs btn-default btn-block mr20" style="margin-top: 8px;">Add New</a>
								</div>
							</div>
						</div>						
					</div>					
				...
				<?php
					...
					if(isset($_REQUEST['image']) && $_REQUEST['image'] != "")
					{
						...
						$sql = mysqli_query($link, "SELECT * FROM sliders WHERE slider_name LIKE '%$search_str%' OR name LIKE '%$search_str%' OR description LIKE '%$search_str%'");
						$targetpage = "sliders.php?image=".$_REQUEST['image']."&page="; //your file name
					}
					else
					{
						$sql = mysqli_query($link, "SELECT * FROM sliders");
						$targetpage = "sliders.php?page="; //your file name
					}
					...
					{
						$search_str = $_REQUEST['image'];
						$query_photos = "SELECT * FROM sliders WHERE slider_name LIKE '%$search_str%' OR name LIKE '%$search_str%' OR description LIKE '%$search_str%' order by display_order asc LIMIT $start ,$limit";
						$res_photos = mysqli_query($link, $query_photos);
					}
					else
					{
						$res_photos = mysqli_query($link, "SELECT * FROM sliders order by display_order asc LIMIT $start ,$limit");
					}
						...
						<div class="mix label1 folder1">
							<div class="panel p6 pbn">
								<div class="of-h">
									...
									<img src="../uploads/store-<?= $app_id; ?>/sliders/<?php echo $row_photos['file_name']; ?>" class="img-responsive" title="<?php echo $row_photos['slider_name']; ?>">
									</div>
									<div class="row table-layout">
										<div class="col-xs-8 va-m pln">
											<h6>
												<?php echo $row_photos['slider_name']; ?> <br/><br/>
												<?php echo formatDateTime($row_photos['created_at']); ?>
											</h6>
										</div>
										<div class="col-xs-4 text-right va-m prn">
											<a href="sliders-update.php?id=<?php echo $row_photos['id']; ?>" style="cursor: pointer; margin-right: 10px;" title="Edit" ><span class="fa fa-pencil fs12 text-info"></span></a>
