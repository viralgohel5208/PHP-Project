...
$sql = "SELECT SA.id, S.store_name, A.first_name, A.last_name, A.email, A.mobile FROM `stores_admin` AS SA INNER JOIN admin AS A ON A.id = SA.admin_id INNER JOIN stores AS S ON S.id = SA.store_id ";
...
while( $row = mysqli_fetch_row($queryRecords) ) {
	...
	$row[6] .= '<a href="stores-admin-assign-update.php?id='.$pri_id.'" class="btn btn-success btn-xs"> &nbsp;<i class="fa fa-pencil"></i></a>';
