...
if(isset($_POST['delete_id']))
{
	...
	$query_3 = "SELECT id, app_id, file_name FROM `testimonials` WHERE id = '" . $id . "'" ;
	...
	if($result_3)
	{
		if ( mysqli_num_rows( $result_3 ) > 0 )
		{
			...
			{
				if($file_name != "" && file_exists("../uploads/store-".$app_id."/testimonials/" . $file_name))
				{
					@unlink("../uploads/store-".$app_id."/testimonials/".$file_name);
				}

				$result = "DELETE FROM testimonials WHERE id = '" . $id . "'";
			...
			<header id="topbar">
				...
				<div class="topbar-right">
					<div class="ml15 ib va-m" id="">
						<a href="testimonials-add.php" class="pl5 btn btn-default btn-sm">
			...
			<section id="content">
						...
						<div class="panel <?php echo $panel_style; ?>">
							<div class="panel-heading">
								<span class="panel-title">
									<span class="fa fa-quote-left"></span><?php echo $page_title; ?>
								</span>
								<div class="pull-right hidden-xs">
									<a href="testimonials-add.php" class="btn btn-xs btn-default btn-block mr20" style="margin-top: 8px;">Add New</a>
								</div>
							</div>
						</div>						
					</div>					
				...
				<?php
					...
					if(isset($_REQUEST['image']) && $_REQUEST['image'] != "")
					{
						...
						$sql = mysqli_query($link, "SELECT * FROM testimonials WHERE name LIKE '%$search_str%' OR name LIKE '%$search_str%'");
						$targetpage = "testimonials.php?image=".$_REQUEST['image']."&page="; //your file name
					}
					else
					{
						$sql = mysqli_query($link, "SELECT * FROM testimonials");
						$targetpage = "testimonials.php?page="; //your file name
					}
					...
					{
						$search_str = $_REQUEST['image'];
						$query_photos = "SELECT * FROM testimonials WHERE name LIKE '%$search_str%' ORDER BY name ASC LIMIT $start ,$limit";
						$res_photos = mysqli_query($link, $query_photos);
					}
					else
					{
						$res_photos = mysqli_query($link, "SELECT * FROM testimonials ORDER BY name ASC LIMIT $start ,$limit");
					}
						...
						<div class="mix label1 folder1">
							<div class="panel p6 pbn">
								<div class="of-h">
									...
									<img src="../uploads/store-<?= $app_id; ?>/testimonials/<?php echo $row_photos['file_name']; ?>" class="img-responsive" title="<?php echo $row_photos['name']; ?>">
									</div>
									<div class="row table-layout">
										<div class="col-xs-8 va-m pln">
											<h6>
												<?php echo $row_photos['name']; ?> <br/><br/>
												<?php echo formatDateTime($row_photos['created_at']); ?>
											</h6>
										</div>
										<div class="col-xs-4 text-right va-m prn">
											<a href="testimonials-update.php?id=<?php echo $row_photos['id']; ?>" style="cursor: pointer; margin-right: 10px;" title="Edit" ><span class="fa fa-pencil fs12 text-info"></span></a>
