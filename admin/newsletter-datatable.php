...
$sql = "SELECT id, email, created_at, status FROM `newsletter` ";
$sqlTot .= $sql;
$sqlRec .= $sql;
...
while( $row = mysqli_fetch_row($queryRecords) ) {
	...
	$row[0] = $k;
	$row[1] = $row[1];
	$row[3] = getUserStatus($row[3]);
	//$row[6] = '<a href="users-update.php?id='.$pri_id.'" class="btn btn-success btn-xs"> &nbsp;<i class="fa fa-pencil"></i></a>';
	$row[4] = '<a onclick="delete_func(\''.$pri_id.'\')" class="btn btn-danger btn-xs next-btn"> &nbsp;<i class="fa fa-times"></i></a>';
	$data[] = $row;
	$k++;
}	
