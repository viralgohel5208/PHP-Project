...
/*
*	Products Listing
* 	URL: http://localhost/ecom/ws/v1/customers-wishlist.php?app_id=1&customer_id=1&auth_token=e8XNj9qQYs7TJgpI4Jyn
*/
...
if($error_code == 0)
{
	$pro_query = "SELECT W.variant_id, W.product_id, P.*, V.* FROM customers_wishlist AS W INNER JOIN products AS P ON P.id = W.product_id INNER JOIN products_variant AS V ON V.id = W.variant_id WHERE W.app_id = '$app_id' AND customer_id = '$customer_id'";
	...
	else
	{
		...
		{
			$error_code = 2; $error_string = 'No products in wishlist';
		}
		else
		{
