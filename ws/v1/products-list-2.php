...
if($error_code == 0)
{
	...
	$pro_query = "SELECT DISTINCT(p.id), p.app_id, p.category_id, p.sku_number, p.brand_name, p.product_name, p.brand_id, p.brand_name, p.product_description, p.file_name, p.offer_type, p.offer_value, p.expires_at, p.total_star_count, p.total_star_customers, p.created_at, p.updated_at, p.status";
