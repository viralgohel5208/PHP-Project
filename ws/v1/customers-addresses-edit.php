...
/*
*	Products Listing
* 	URL: http://localhost/ecom/ws/v1/customers-addresses-edit.php?app_id=1&customer_id=1&auth_token=e8XNj9qQYs7TJgpI4Jyn&address_id=1&address_name=Office Address&first_name=Shirish&last_name=Makwana&email=shirishm.makwana@gmail.com&mobile=9924400799&address=Royal Complex&city_id=1&state_id=1&country_id=99&landmark=Bhutkhana&pincode=360002&latitude=&longitude=
*/
...
else if(!isset($_REQUEST['address_id']) || !isset($_REQUEST['address_name']) || !isset($_REQUEST['first_name']) || !isset($_REQUEST['last_name']) || !isset($_REQUEST['email']) || !isset($_REQUEST['mobile']) || !isset($_REQUEST['address']) || !isset($_REQUEST['city_id']) || !isset($_REQUEST['state_id']) || !isset($_REQUEST['country_id']) || !isset($_REQUEST['landmark']) || !isset($_REQUEST['pincode']) || !isset($_REQUEST['latitude']) || !isset($_REQUEST['longitude']))
{
...
else
{
	...
	if($address_id =="" || $address_name =="" || $first_name == "" || $last_name == "" || $mobile == "" || $address == "")
	{
		$error_code = 2; $error_string = "Please enter all the mandatory details";
	}
	else if($email != "" && !preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/i", $email))
	{
		$error_code = 2; $error_string = "Invalid email address";
	}
	else if (!preg_match("/^[0-9]*$/", $mobile))
	{
		$error_code = 2; $error_string = "Mobile no is invalid";
	}
}
...
if($error_code == 0)
{
	...
	{
		$error_code = 4; $error_string = 'Address details does not found';
	}
	else
	{
...
if($error_code == 0)
{
	...
	{
		$error_code = 6; $error_string = "Please use different address name";
	}
	else
	{
		//$row = mysqli_fetch_assoc($result);
...
if($error == "")
{
	...
	else
	{
		...
		{
			$error_code = 6; $error_string = "Invalid city details";
		}
		else
		{
