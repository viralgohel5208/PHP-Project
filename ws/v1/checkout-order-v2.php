		...
		while($row_2 = mysqli_fetch_assoc($result2))
		{
			...
			if($stock_amount > 0 && $stock_amount > $quantity)
			{
				...
				else if($customer_offer == 2)*/
				{
					...
					$query5 = "INSERT INTO `order_details`(`app_id`, `order_id`, `product_id`, `variant_id`, `quantity`, `offer_reference`, `price_raw`, `price_gst`, `price_finale`, `price_discounted`, `price_total`, `status`) VALUES ('$app_id', '$order_id', '$product_id', '$variant_id', '$quantity', '$offer_value', '$price_gst', '$price_raw', '$price_finale', '$price_discounted', '$price_total', '0')";
...
if($error_code == 0)
{
	...
	$query6 = "UPDATE `orders` SET `price_raw` = '$ord_price_gst', `price_gst` = '$ord_price_raw', `price_finale` = '$ord_price_finale', `price_discounted` = '$ord_price_discounted', `price_delivery_charge` = '$delivery_charge', `price_total` = '$ord_price_total' WHERE id = $order_id";
