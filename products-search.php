...
if(isset($_GET['product-name']))
{
	...
	else*/
	{
		...
		if(isset($_REQUEST['product-name'])) {
			$product_name 	= escapeInputValue($_REQUEST['product-name']);
		} else {
			$product_name = "";
		}
		
		if(isset($_REQUEST['brand'])) {
			$brand_name 	= escapeInputValue($_REQUEST['brand']);
		} else {
			$brand_name = "";
		}
...
{
	header("Location:categories1.php");
	exit;
}
				...
				<div class="row">
					<div class="col-xs-12">
						<ul>
							...
							<li class="category13"><strong>Search Products</strong></li>
						</ul>
					</div>
				</div>
			</div>
			...
			<div class="container">
				<div class="row">
					<aside class="left sidebar col-sm-3 col-xs-12">
						...
						<div class="block popular-tags-area category-sidebar">
								...
								<form class="navbar-form" action="products-search.php">
									<div id="search">
										<div class="input-group">
											<input name="product-name" placeholder="Search" class="form-control" type="text" style="width: 79%" value="<?php if(isset($_GET['product-name'])) { echo $_GET['product-name']; } ?>">
						...
						<div class="block popular-tags-area category-sidebar">
							...
							<div class="tag">
								<ul>
									<?php 
$get_vars = $_GET;
if(isset($get_vars['brand'])) { unset($get_vars['brand']); }
...
$query_string = implode("&", $query_string);
//echo '<pre>'; print_r($get_vars); echo '</pre>';
$targetpage2 = $current_url."?".$query_string."&brand=";
//echo targetpage;
//exit;
									echo '<li><a href="'.$targetpage2.'0">All</a></li>';
									
									$query = "SELECT id, brand_name FROM brands WHERE app_id = $app_id AND brand_name != '-' ORDER BY RAND() LIMIT 20"; 
									$result = mysqli_query($link, $query);
									while($row = mysqli_fetch_Assoc($result))
									{
										//if($row['brand_name'] == "-") { $row['brand_name'] = "Other"; }
										echo '<li><a href="'.$targetpage2.$row['id'].'">'.$row['brand_name'].'</a></li>';
									}
									?>
								</ul>
							</div>
					...
					<div class="col-main col-sm-9">
						<div class="page-title">
							<h2>Search Products <?php if(isset($_GET['product-name'])) { echo " - ".$_GET['product-name']; } ?> </h2>
						</div>
											...
											<div class="product-thumbnail">
												...
												<div class="pr-info-area">
													<div class="pr-button" style="left: 60%">
														<div class="mt-button">
														<?php $i = 0; foreach($variants as $item) {
														if($item['wishlist'] == 1) { $heart_string = "fa-heart"; }
